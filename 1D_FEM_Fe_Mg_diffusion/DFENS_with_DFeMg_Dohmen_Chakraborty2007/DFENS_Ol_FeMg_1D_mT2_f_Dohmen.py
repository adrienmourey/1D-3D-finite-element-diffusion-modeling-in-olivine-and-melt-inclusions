...
def D(C, T_K, P_Pa, fO2_Pa, Ea, aniso):
    x1 = 10**(-9.21)
    x2 = ((fO2_Pa/(1e-7))**(1.0/6.0))
    x3 = (Constant(10.0)**(Constant(3.0)*(Constant(0.9)-C)))
    x4 = np.exp(-(Ea + ((7e-6)*(P_Pa - 1e5)))/(8.314*T_K))
    
    D_001 = Constant(x1)*Constant(x2)*x3*Constant(x4)
...
def mod_diff(cube, nparams):
    ...
    t1, t2, T1, T2, fe3, P, Ea, comp0, comp1, comp2, intf1, intf2 = cube[0], cube[1], cube[2], cube[3], cube[4], cube[5], cube[6], cube[7], cube[8], cube[9], cube[10], cube[11]
    ...
    #plt.plot(rcoords, dat_t2[::-1], color = 'red')
    #plt.plot(rcoords, inicon_1[::-1], color = 'blue', linestyle = '--')
    #plt.plot(rcoords, inicon_2[::-1], color = 'red', linestyle = '--')
    #plt.ylabel('XFo')
    #plt.xlabel('Distance (um)')
    #plt.savefig('OL49_C_model_{0}_{1}_{2}_{3}.png'.format(t1/(86400.0), t2/(86400.0), T1 -273, T2-273))
    #plt.close()
...
#        Fixed = True
    ...
    parameters = ["t1", "t2", "T1", "T2", "lnfO2", "P", "Ea", "C0", "C1", "C2", "b1", "b2"]
    ...
    pcube[6,:] = [201000.0, 20000.0] # Activation energy Ea
    pcube[7,:] = [obs_mpar['C0'][0], obs_mpar['C0'][1]]     # Composition zone 0
    pcube[8,:] = [obs_mpar['C1'][0], obs_mpar['C1'][1]]     # Composition zone 1
    pcube[9,:] = [obs_mpar['C2f'][0], obs_mpar['C2f'][1]]     # Composition zone 2
    pcube[10,:] = [obs_mpar['step1'][0], obs_mpar['step1'][1]]     # Boundary position 1
    pcube[11,:] = [obs_mpar['step2'][0], obs_mpar['step2'][1]]     # Boundary position 2
