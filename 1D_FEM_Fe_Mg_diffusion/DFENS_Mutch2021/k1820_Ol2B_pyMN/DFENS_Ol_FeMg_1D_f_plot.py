...
def mod_diff(cube, nparams):
    ...
    plt.errorbar(dist, obs['XFo'].values, yerr = obs['Fo_stdev'].values ,fmt='o', color='grey', label='data', zorder = -3)
    plt.plot(rcoords, dat_t1[::-1], color = 'blue', zorder = -1)
    plt.plot(rcoords, inicon_1[::-1], color = 'blue', linestyle = '--', zorder = -1)
    plt.ylabel('XFo')
    plt.xlabel('Distance (um)')
    plt.savefig('ol_test_model_{0}_{1}.png'.format(t1/(86400.0), T1 -273))
    plt.close()
...
#        Fixed = True
    ...
    pcube[6,:] = [obs_mpar['step_microns'][0], obs_mpar['step_microns'][1]]     # Boundary position 1
