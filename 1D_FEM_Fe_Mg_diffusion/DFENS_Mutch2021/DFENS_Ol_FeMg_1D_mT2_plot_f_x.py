...
def mod_diff(cube, nparams):
    print('running')
    t1, t2, T1, T2, fe3, P, comp0, comp1, comp2, intf1, intf2, DFo = cube[0], cube[1], cube[2], cube[3], cube[4], cube[5], cube[6], cube[7], cube[8], cube[9], cube[10], cube[11:nparams]

    t1 *= (86400.0) # convert time into seconds
    t2 *= (86400.0) # convert time into seconds
    ...
    for c in cells(mesh):
        if (c.midpoint().x() > intf1):
            cmarker[c] = 0
        elif (c.midpoint().x() < intf1) and (c.midpoint().x() > intf2):
            cmarker[c] = 1
        else:
            cmarker[c] = 2
    ...
    for f in facets(mesh):
        ...
        if len(cc) == 2 and cmarker[cc[0]] != cmarker[cc[1]]:
            if cmarker[cc[0]] < 1 or cmarker[cc[1]] < 1:
                fmarker[f] = 1
            elif cmarker[cc[0]] > 1 or cmarker[cc[1]] > 1:
                fmarker[f] = 2
    ...
    for c in cells(mesh): # compute dofs in the domains
        if cmarker[c] == 0:
            ...
            d2_dofs.extend(dofmap.cell_dofs(c.index()))

    # unique
    d2_dofs = list(set(d2_dofs))
    ...
    # Repeat for second phase

    D2_fo = D(C_mid_fo, T2_i, P, lnfO2, Constant(DFo[0]), Constant(DFo[1]), Constant(DFo[2]), Constant(DFo[3]), Constant(DFo[4]), Constant(DFo[5]), 6.0)
    F2_fo = S_fo*(C1_fo-C0_fo)*dx + dT2*(inner(D2_fo*grad(S_fo), grad(C_mid_fo)))*dx
    ...
    C0_fo.vector()[d2_dofs] = Constant(comp1)

    inicon_1 = C0_fo.vector().get_local()

    # Boundary conditions
    #Cbc0 = DirichletBC(Q, Constant(comp1), edge_boundary1) 
    Cbc0 = DirichletBC(Q, Constant(comp1), fmarker, 2)
    Cbcs = [Cbc0]
    ...
    C0_fo.vector()[d2_dofs] = Constant(comp2)

    inicon_2 = C0_fo.vector().get_local()

    # Boundary Conditions
    Cbc0 = DirichletBC(Q, Constant(comp2), edge_boundary2) 
    Cbcs = [Cbc0]

    # Run second diffusion model for t2 and T2

    i = 0
    
    while i < t2:
        solve(F2_fo==0, C1_fo, Cbcs)
        C0_fo.assign(C1_fo)

        i += dt2 # Need to decide on time increment    
        
    rcoords = mesh.coordinates()
        
    gl_mod = modc(C0_fo.vector().get_local(), rcoords, dist)

    dat_t2 = C0_fo.vector().get_local()

    
    plt.errorbar(dist, obs['XFo'].values, yerr = obs['Fo_stdev'].values ,fmt='o', color='grey', label='data')
    plt.plot(rcoords, dat_t1[::-1], color = 'blue', label = 'Model t1')
    plt.plot(rcoords, dat_t2[::-1], color = 'red', label = 'Model t2')
    plt.plot(rcoords, inicon_1[::-1], color = 'blue', linestyle = '--', label = 'Inicon 1')
    plt.plot(rcoords, inicon_2[::-1], color = 'red', linestyle = '--', label = 'Inicon 2')
    plt.ylabel('XFo')
    plt.xlabel('Distance (um)')
    plt.legend(loc = 0, fontsize= 'xx-small')
    plt.savefig(f_out + 'model_fits.pdf')
    plt.close()
    
    # Save the model data to a file

    df_dd = pd.DataFrame({'Distance (um)': rcoords[:,0], 'Fo Final Model Fit': dat_t2[::-1], 'Fo Inicon 1': inicon_1[::-1], 'Fo Inicon 2': inicon_2[::-1], 'Fo Model t1': dat_t1[::-1], 'Fo Model t2': dat_t2[::-1]})
    
    df_dd.to_csv(f_out + 'model_fits.csv', sep=',') 
...
if __name__ == "__main__":
    ...
    obs1 = pd.read_csv(f_dat)
    
    # filter observations based on zone
    
    obs = obs1[obs1['Zone'] == 1]
...
#        Fixed = True
    ...
    parameters = ["t1", "t2", "T1", "T2", "lnfO2", "P", "C0", "C1", "C2", "b1", "b2", "lnD0_Fo", "clnfO2_Fo", "cXFo_Fo", "cT_i_Fo", "cP_Fo", "cT_iP_Fo"]
    ...
    # setup prior cube
    ptype = ["MG"] * n_dim
    ptype[0:2] = ["LU"]*2   #type of distribution LU = ln uniform, U = uniform
    ptype[2:6] = ["G"]*4
    ptype[6:11] = ["U"]*5
    ...
    pcube[0,:] = [np.log10(obs_mpar['time1'][0]), np.log10(obs_mpar['time1'][1])]     # time 1 (days)
    pcube[1,:] = [np.log10(obs_mpar['time2'][0]), np.log10(obs_mpar['time2'][1])]     # time 2 (days)
    pcube[2,:] = [obs_mpar['T1'][0], obs_mpar['T1'][1]] # temperature 1 (C)
    pcube[3,:] = [obs_mpar['T2'][0], obs_mpar['T2'][1]] # temperature 2 (C)
    pcube[4,:] = [obs_mpar['lnfO2_bar'][0], obs_mpar['lnfO2_bar'][1]]     # ln fO2 in bars! - natural log
    pcube[5,:] = [obs_mpar['P_kbar'][0], obs_mpar['P_kbar'][1]]     #P kbar
    pcube[6,:] = [obs_mpar['C0'][0], obs_mpar['C0'][1]]     # Composition zone 0
    pcube[7,:] = [obs_mpar['C1'][0], obs_mpar['C1'][1]]     # Composition zone 1
    pcube[8,:] = [obs_mpar['C2f'][0], obs_mpar['C2f'][1]]     # Composition zone 2
    pcube[9,:] = [obs_mpar['step1'][0], obs_mpar['step1'][1]]     # Boundary position 1
    pcube[10,:] = [obs_mpar['step2'][0], obs_mpar['step2'][1]]     # Boundary position 2
    pcube[11:,0] = np.array([-6.755, 2.244e-1, -7.181, -2.674e4, -5.213e-10, -1.028e-7]) # DFo
