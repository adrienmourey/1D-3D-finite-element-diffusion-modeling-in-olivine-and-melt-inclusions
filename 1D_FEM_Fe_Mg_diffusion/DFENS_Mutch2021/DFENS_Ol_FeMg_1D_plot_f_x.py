...
def mod_diff(cube, nparams):
    ...
    plt.plot(rcoords, dat_t1[::-1], color = 'blue', label = 'Model fit')
    plt.plot(rcoords, inicon_1[::-1], color = 'blue', linestyle = '--', label = 'Initial Condition')
    plt.ylabel('XFo')
    ...
    df_dd = pd.DataFrame({'Distance (um)': rcoords[:,0], 'Fo Final Model Fit': dat_t1[::-1], 'Fo Inicon 1': inicon_1[::-1]})
