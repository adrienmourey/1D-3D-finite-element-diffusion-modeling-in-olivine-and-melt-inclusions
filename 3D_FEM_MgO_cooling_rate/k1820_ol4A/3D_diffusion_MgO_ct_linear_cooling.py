...
# Import mesh here
mesh1 = Mesh()
hdf = HDF5File(mesh1.mpi_comm(), './{0}.h5'.format(f_mesh), "r")
hdf.read(mesh1, "/mesh", False)
domains = MeshFunction('size_t', mesh1, mesh1.topology().dim())
...
xdmf_femg =  XDMFFile("./{0}_output_FeMg_test_cooling.xdmf".format(f_mesh))
...
cell_dim = mesh1.topology().dim()
cmarker1 = MeshFunction("size_t", mesh1, cell_dim, 0)
fmarker1 = MeshFunction("size_t", mesh1, cell_dim - 1, 0)

vmarker1 = MeshFunction("size_t", mesh1, cell_dim - 2, 0)
tmarker1 = MeshFunction("size_t", mesh1, cell_dim - 2, 0)

bmarker1 = MeshFunction("bool", mesh1, cell_dim, False)
mimarker1 = MeshFunction("bool", mesh1, cell_dim, False)
...
# Refine the mesh on bondaries and within the melt inclusions

# Optional mesh refinement 

mesh = refine(mesh1, bmarker1)
cmarker = adapt(cmarker1, mesh)
fmarker = adapt(fmarker1, mesh)
print('Mesh refined')
